 ...
 * to ONE of the two lists — BUILT_SCREENS if it is a sidebar item, DETAIL_ROUTES
 * if it is reached some other way (clicking a row, the account menu, a button).
 *
 * One of these, not both, and it matters: routes are generated by walking
 * `config.nav`, so BUILT_SCREENS is only ever consulted for a path that is
 * already a nav item. Put an off-nav screen there and it compiles, imports and
 * renders nowhere — every link to it reaches Page not found. That happened to
 * individual/Receipts, which is why it is in DETAIL_ROUTES below.
 *
 * Both are plain lists, so if two of us add to them at once git reports a
...
'school_admin:': <SchoolAdminDashboard />,
  ...
  /* A SIDEBAR ITEM, so it belongs in this list and not in DETAIL_ROUTES —
     routes are generated by walking `config.nav`, and the two lists are read
     for opposite kinds of screen. db/111 built the subscription as a section
     inside Settings › Payments; this is that same section given the nav entry
     it should always have had. */
  'individual:subscription': <IndividualSubscription />,
...
> = {
  ...
  // --- Saurab: individual, parent, platform_admin ---
  // Receipts is deliberately not a sidebar item: somebody wants it twice a
  // year, at tax time, and a permanent tab for it would push the things they
  // came for down the list. It is reached from the account menu and from the
  // purchase list on their home page.
  //
  // It has to be HERE rather than in BUILT_SCREENS, and that is the whole bug
  // this entry fixes. Routes are generated from `config.nav` alone, so a
